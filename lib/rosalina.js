...
class Rosalina {
  ...
  static writeError(res, headers, error) {
    ...
    res.write("An error was found during while processing the request.\n");
  ...
  */
  ...
  constructor(remote) {
    debug('new Rosalina instance with remote host as %s', remote);
  ...
  logic() {
    ...
    return (function (req, res) {
      ...
      Parser.parse(req).then((form) => {
        ...
        debug('received response %o, %o, %o', remote.data, remote.headers, remote.status);
        ...
        if(!err.response) {
          debug('assuming internal error because no response from request');
          writeError(boom.wrap(err, 500));
        } else {
          debug('connection problem error: %d %o', err.response.status, err.response);
